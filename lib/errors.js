...
class NotFoundError extends ExtendedError {
  constructor(message, options = {}){
    ...
    this.statusCode = this.status = options.status || 404
...
class ValidationError extends ExtendedError {
  ...
  constructor(message, options = {}){
    super(message, options)
    this.statusCode = this.status = options.status || 400
...
class HttpError extends ExtendedError {
  ...
  constructor(status, message, options = {}){
    ...
    // Add a status
    this.statusCode = this.status = status
...
class KeyError extends ExtendedError {
  ...
  constructor( message, options = {} ){
    ...
    this.statusCode = this.status = ( options.status ) ? options.status : 404
  }
...
// ### Exports
module.exports = {
  httpErrorMap, HttpError, ExtendedError, ValidationError, KeyError, NotFoundError
}
